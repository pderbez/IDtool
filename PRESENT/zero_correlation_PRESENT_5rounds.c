...
uint64_t* generateSubkeys(uint64_t keyHigh, uint16_t keyLow){
...
uint8_t encrypt(uint64_t state, uint64_t* subkeys){
    int i, j;
    ...
    return (state &1) ^(state >>16 &1) ^ (state>>48 & 1);
}
...
int main(){
    ...
    uint64_t* subkeys = generateSubkeys(key0,key1);

    for (uint8_t i =0;i<100;i++){
        sum = 0;
        c = rand();
        for (uint64_t x=0; x<(((uint64_t)1) <<12); x++) {
            //calling the encrypt function
            plaintext = c ^ (x<<40);
            sum^= encrypt(plaintext, subkeys);
