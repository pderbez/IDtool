...
def getVectorSpaceLAT(S, m):
    LAT = S.linear_approximation_table()
    ALAT = {}
    V = VectorSpace(GF(2), m)
    for diff_input in range(LAT.nrows()):
        diffs_output = [convert_to_vector(diff_output, m) for diff_output in range(LAT.nrows()) if LAT[diff_input, diff_output] != 0]
        ALAT[convert_to_string(diff_input,m)] = V.subspace(diffs_output)
...
def getModel(S, name):
    print(f"=============== {name} ==================")
    m = S.input_size()
    A = getVectorSpaceLAT(S, m)
   
    f = open("S_"+name+"_Semi_Min_Lat.esp", 'w')
    f.write(f".i {2*m}\n.o 1\n.p {len(A)*(2**m)}\n")
