...
def my_callback(model, where):
    ...
    if where == gp.GRB.Callback.MIPSOL:
        ...
        if s == gp.GRB.INFEASIBLE:
            ...
            Obj = 0 
            for rM in range(rD//2) :
                Obj = SIMECK(rD-1,n,rM, input=target_i, output=target_o) 
                if Obj > 0.5 :
                    break

                Obj = SIMECK(rD-1,n,rD-rM-2, input=target_i, output=target_o) 
                if Obj > 0.5 :
                    break
            print(rM,Obj, "indirect  contradictions")
